  ...
  _createClass(Client, [{
    ...
    value: function textRequest(text, callback, options) {
      ...
      if (!TOKEN) {
        ...
        _superagent2.default.post('https://api.recast.ai/v1/request').set('Authorization', 'Token ' + TOKEN).send({ text: text }).end(function (err, res) {
          if (err) {
            return callback(null, new _error.RecastError(err.message));
          } else {
    ...
    value: function fileRequest(file, callback, options) {
      ...
      if (!TOKEN) {
        ...
        _superagent2.default.post('https://api.recast.ai/v1/request').attach('voice', file).set('Authorization', 'Token ' + TOKEN).set('Content-Type', '').end(function (err, res) {
          if (err) {
            return callback(null, new _error.RecastError(err.message));
          } else {
