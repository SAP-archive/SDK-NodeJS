...
var Client = function () {
  ...
  _createClass(Client, [{
    ...
    value: function textRequest(text, callback, options) {
      ...
      if (!TOKEN) {
        return callback(new _error2.default('Token is missing'), null);
      } else {
        var req = _superagent2.default.post(_constants2.default.API_ENDPOINT).set('Authorization', 'Token ' + TOKEN);

        if (PROXY) {
          req.proxy(PROXY);
        }

        req.send(params).end(function (err, res) {
          if (err) {
            return callback(new _error2.default(err.message), err);
          } else {
            return callback(null, new _response2.default(res.body));
          }
        });
      }
    }
    ...
    value: function fileRequest(file, callback, options) {
      ...
      if (!TOKEN) {
        ...
        var req = _superagent2.default.post(_constants2.default.API_ENDPOINT).attach('voice', file).set('Authorization', 'Token ' + TOKEN).set('Content-Type', '');
